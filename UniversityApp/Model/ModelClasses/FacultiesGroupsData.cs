...
namespace Model.ModelClasses
{
    public class FacultiesGroupsData : ADataBase
    {
        ...
        public bool checkFacultiesGroups(List<string> data)
        {
            MySqlCommand command_groups_num = new MySqlCommand($"SELECT COUNT(*) FROM `{name_table}`", connection);

            openConnection();
            int numRowsGroups = Convert.ToInt32(command_groups_num.ExecuteScalar());
            closeConnection();

            MySqlCommand command = new MySqlCommand($"SELECT faculties_groups.num_faculty, faculties_groups.num_group FROM `{name_table}`", connection);
            List<string[]> data_faculties_groups = new List<string[]>();
            ...
            for (int i = 0; i < numRowsGroups; ++i)
            {
                if (reader.Read())
                {
                    data_faculties_groups.Add(new string[num_cell_data]);

                    for (int a = 0; a < num_cell_data; ++a)
                    {
                        data_faculties_groups[data_faculties_groups.Count - 1][a] = reader[a].ToString();
                    }
                }
            }
            ...
            if (data.Count() > 1)
            {
                ...
                {
                    if (data_faculties_groups[i][1] == data[1])
                    {
                        return false;
                    }
                }
        ...
        public override List<string[]> getAllData()
        {
            ...
            MySqlCommand command = new MySqlCommand($"SELECT faculties_groups.id, faculties.name, groups.name FROM `{name_table}` JOIN faculties ON faculties_groups.num_faculty = faculties.id " +
